...
void VGAController::writeScreenPixel(int x, int y, uint16_t data)
{
  uint8_t * row = (uint8_t*) m_viewPort[y];
  uint16_t c = data;
  //RGB565->RGB888->RGB222
  ...
  G = G >> 6;
  B = B >> 6;
  uint8_t rgb222 = (B << VGA_BLUE_BIT) | (R << VGA_RED_BIT) | (G << VGA_GREEN_BIT);
  VGA_PIXELINROW(row, x) = rgb222 | m_HVSync;
}

// no bounds check is done!
void VGAController::writeScreenPalette(Rect const & rect, const uint8_t *data[], uint16_t * palette)
{
  for (int y = rect.Y1; y <= rect.Y2; ++y) {
    uint8_t * row = (uint8_t*) m_viewPort[y];
    for (int x = rect.X1; x <= rect.X2; ++x){
      uint16_t c = palette[data[y - rect.Y1][x - rect.X1]];
      //uint16_t c = palette[c];
      //RGB565->RGB888->RGB222
      uint8_t R = ((c >> 11) & 0x1F) << 3;
      uint8_t G = ((c >> 5 ) & 0x3F) << 2;
      uint8_t B = (c & 0x1F) << 3;
      ...
      B = B >> 6;
      uint8_t rgb222 = (B << VGA_BLUE_BIT) | (R << VGA_RED_BIT) | (G << VGA_GREEN_BIT);
      //uint8_t rgb222 = (rgb565>>3 & 0x3) | (rgb565>>7 & 0xC) | (rgb565>>9 & 0x30);
      VGA_PIXELINROW(row, x) = rgb222 | m_HVSync;
      }
  }
}
