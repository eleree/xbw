...
static void custom_blit(bitmap_t *bmp, int num_dirties, rect_t *dirty_rects) {
	if(bmp == NULL)
		printf("custom_blit bmp is null \r\n");
...
//		xQueueReceive(vidQueue, &bmp, portMAX_DELAY);//skip one frame to drop to 30
		...
		vga_write_frame(x, y, DEFAULT_WIDTH, DEFAULT_HEIGHT, (const uint8_t **)bmp->line);
		vTaskDelay(1);
	}
}
...
void osd_getinput(void)
{
	...
	#else
		//const int ev[8]={
		//	event_joypad1_b,event_joypad1_right,event_joypad1_left,event_joypad1_down,
 		//	event_joypad1_up,event_joypad1_start,event_joypad1_b,event_joypad1_a
		//};
		//const int ev[8]={
		//	event_joypad1_right,event_joypad1_left,event_joypad1_down,event_joypad1_up,
 		//	event_joypad1_start,event_joypad1_select,event_joypad1_b,event_joypad1_a
		//	};
		const int ev[16]={
			event_joypad1_right,event_joypad1_left,event_joypad1_down,event_joypad1_up,
 			event_joypad1_start,event_joypad1_select,event_joypad1_b,event_joypad1_a,
			event_joypad2_right,event_joypad2_left,event_joypad2_down,event_joypad2_up,
 			event_joypad2_start,event_joypad2_select,event_joypad2_b,event_joypad2_a
		};
	static int oldb=0xffff;
	int b=joystickReadInput();
	#endif
	int chg=b^oldb;
	int x;
	oldb=b;
...
int osd_init()
{
	...
	if(vidQueue == NULL)
	{
		printf("create vidQueue fail \r\n");
		while(1);
	}
	xResult = xTaskCreatePinnedToCore(&videoTask, "videoTask", 1096, NULL, 5, NULL, 1);
	if( xResult != pdPASS )
	{
		printf("create videoTask fail \r\n");
		while(1);
	}
	osd_initinput();
	return 0;
